...
class TestParseErrorMap:
    def test_parses_steps_and_errors(self):
        ...
        assert parse_error_map(msg) == [
            {"step": "ConditionBranchStep2", "errors": ["InvalidEntity", "InvalidRelationship"]},
...
def _wf_row(wid, statecode=0):
    return {"workflowid": wid, "name": f"SLA wf {wid[:4]}", "type": 1, "statecode": statecode}
...
class TestActivateSla:
    ...
    def test_already_active_workflow_skipped(self, backend):
        ...
        assert result["sla_activated"] is True
        assert [w["status"] for w in result["workflows"]] == ["already_active", "activated"]
    ...
    def test_compile_failure_blocks_sla_and_reports_structured_errors(self, backend):
        ...
        with requests_mock.Mocker() as m:
            _mock_sla_reads(m, backend)
            m.patch(
                backend.url_for(f"workflows({_WF1})"),
                status_code=400,
    ...
    def test_dry_run_returns_preview_and_never_patches(self, profile):
        ...
        assert result["would_activate"] == [{"workflow_id": _WF2, "name": f"SLA wf {_WF2[:4]}"}]
        assert result["already_active"] == [{"workflow_id": _WF1, "name": f"SLA wf {_WF1[:4]}"}]
    ...
    def test_slaitems_paging_follows_next_link(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.get(
                backend.url_for("slaitems"),
                [
                    {"json": {**_items_payload(), "@odata.nextLink": next_url}},
                ],
            )
            for wid in (_WF1, _WF2, wf3):
                m.get(backend.url_for(f"workflows({wid})"), json=_wf_row(wid, 1))
        ...
        "workflows": [
            {
                ...
                "errors": [{"step": "ConditionBranchStep2", "errors": ["InvalidEntity"]}],
            },
        ],
    }
...
def _created_header(entity_set, guid):
    return {
        "OData-EntityId": f"https://crm.contoso.local/contoso/api/data/v9.1/{entity_set}({guid})"
    }
...
class TestCreateSla:
    def test_posts_record_and_reports_already_enabled(self, backend):
        ...
        with requests_mock.Mocker() as m:
            post = m.post(
                backend.url_for("slas"), status_code=204, headers=_created_header("slas", _NEW_SLA)
            )
            m.get(
                backend.url_for("EntityDefinitions(LogicalName='incident')"),
    ...
    def test_business_hours_bound_as_lookup(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            create_sla(backend, name="Gold", entity="incident", business_hours_id=_BUSINESS_HOURS)
        body = post.last_request.json()
    ...
    def test_dry_run_previews_without_posting(self, profile):
        ...
        with requests_mock.Mocker() as m:
            m.get(
                dry.url_for("EntityDefinitions(LogicalName='incident')"),
    ...
    def test_requires_conditions(self, backend):
        ...
        with requests_mock.Mocker() as m:
            with pytest.raises(D365Error, match="applicable_when is required"):
                add_kpi(
                    backend, sla_id=_SLA_ID, kpi="k", applicable_when="", success_criteria=_SUCCESS
                )
            with pytest.raises(D365Error, match="success_criteria is required"):
                add_kpi(
                    backend, sla_id=_SLA_ID, kpi="k", applicable_when=_FETCH, success_criteria=""
                )
        assert m.request_history == []
...
class TestSlaActivateCommand:
    def test_json_happy_path(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            sla_cmd.sla_mod, "activate_sla", lambda backend, sid, **kw: _success_result()
        )
        from crm.cli import cli

        result = CliRunner().invoke(cli, ["--json", "--profile", "t", "sla", "activate", _SLA_ID])
        assert result.exit_code == 0
        envelope = json.loads(result.output)
        assert envelope["ok"] is True
    ...
    def test_compile_failure_exits_nonzero_with_ui_message(self, monkeypatch, tmp_path):
        ...
        monkeypatch.setattr(
            sla_cmd.sla_mod, "activate_sla", lambda backend, sid, **kw: _failure_result()
        )
        from crm.cli import cli
        ...
        assert envelope["data"]["workflows"][0]["errors"][0]["step"] == "ConditionBranchStep2"
