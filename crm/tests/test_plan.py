...
def test_build_plan_header_captures_identity_and_intent(backend):
    ...
    assert header["intent"] == {"prune": True, "allow_data_loss": False, "stage_only": True}
...
def test_build_plan_verdict_records_map_buckets_and_carry_diff(backend):
    report = _report(
        ...
        pruned=[{"kind": "view", "name": "Stale", "deleted": False, "would_prune": True}],
    ...
    assert by_name["contoso_Code"]["diff"] == {"display_name": {"old": "Code", "new": "Code #"}}
...
def test_build_plan_records_ui_kind_changed_field_sets(backend):
    ...
    report = _report(
        updated=[
            {
                "kind": "form",
                "name": "contoso_project main form",
...
def test_preflight_refuses_org_mismatch(backend):
    ...
    with pytest.raises(D365Error, match="organization"):
        plan_mod.preflight_plan(plan, backend, organization_id="a-different-org", base_dir=None)
...
def test_preflight_url_mismatch_is_a_warning_not_a_refusal(backend):
    ...
    warnings = plan_mod.preflight_plan(plan, backend, organization_id="org-guid-123", base_dir=None)
...
def test_preflight_refuses_replace_blocked_plan(backend):
    plan = _plan(
        verdicts=[
            {"kind": "entity", "name": "x", "verdict": "replace_blocked", "reason": "ownership"}
        ]
    )
...
def test_preflight_refuses_failed_plan(backend):
    plan = _plan(
        verdicts=[
            {"kind": "webresource", "name": "x", "verdict": "failed", "error": "cannot read file"}
        ]
    )
    with pytest.raises(D365Error, match="not executable"):
        plan_mod.preflight_plan(plan, backend, organization_id="org-guid-123", base_dir=None)
        ...
        updated=[
            {
                ...
                "name": "contoso_Project",
                "diff": {
                    "display_name": {"old": "P", "new": "Project"},
...
def test_diff_plan_flags_app_action_change_from_live_ui_edit(backend):
    ...
    live = _report(
        updated=[{"kind": "app", "name": "cwx_crmworx", "diff": {"fields": ["sitemap:converged"]}}]
    )
    div = plan_mod.diff_plan(plan, live)
