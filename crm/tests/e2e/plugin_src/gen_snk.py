...
def _gen_pkcs1_der() -> bytes:
    ...
    pem = subprocess.run(["openssl", "genrsa", "2048"], check=True, capture_output=True).stdout
...
def _private_key_blob(
    n: int, e: int, d: int, p: int, q: int, dp: int, dq: int, qinv: int, bits: int
) -> bytes:
