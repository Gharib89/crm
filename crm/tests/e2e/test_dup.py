...
def test_dup_list(cli):
    ...
    assert result.returncode == 0, f"dup list failed:\n{result.stderr}\nstdout: {result.stdout}"
...
@covers("dup create", "dup add-condition", "dup publish", "dup unpublish", "dup check", "dup get")
@pytest.mark.slow
def test_dup_lifecycle(backend, cli, request, unique, ephemeral_solution):
    ...
    assert result.returncode == 0, f"dup get failed:\n{result.stderr}\nstdout: {result.stdout}"
    ...
    assert result.returncode == 0, f"dup check failed:\n{result.stderr}\nstdout: {result.stdout}"
