...
def _sitemap_raw(cli, sitemap_id: str) -> str:
    ...
    got = _data(
        cli(["--json", "query", "odata", f"sitemaps({sitemap_id})", "--select", "sitemapxml"])
    )
...
def test_sitemap_live_edit_lifecycle(cli, unique, ephemeral_solution):
    ...
    created = _data(
        cli(
            [
                ...
                "cwxarea:CWX Area",
                "--group",
                "cwxarea/cwxgrp:CWX Group",
                "--subarea",
                "cwxarea/cwxgrp:entity=account:Accounts",
                "--unique-name",
                uniq,
                "--no-publish",
                "--solution",
    ...
    try:
        ...
        sub = next(s for s in root.iter("SubArea") if s.get("Id") == "cwx_opscontacts")
        ...
        deleted = _data(cli(["--json", "entity", "delete", "sitemaps", sitemap_id, "--yes"]))
...
def test_sitemap_set_localized_title_description(cli, unique, ephemeral_solution):
    ...
    try:
        ...
        area = next(
            a for a in _sitemapxml(cli, sitemap_id).iter("Area") if a.get("Id") == "cwxarea"
        )
        ...
        assert [t.get("Title") for t in area.findall("Titles/Title")] == ["Selling"], ET.tostring(
            area, "unicode"
        )
    finally:
        deleted = _data(cli(["--json", "entity", "delete", "sitemaps", sitemap_id, "--yes"]))
