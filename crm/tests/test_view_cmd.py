...
def test_edit_columns_bad_width_spec_is_bad_param(monkeypatch):
    ...
    res = CliRunner().invoke(
        ...
        [
            ...
            "My View",
            "--width",
            "name",  # missing colon → BadParameter
        ],
    )
...
def test_edit_columns_add_calls_core(monkeypatch):
    ...
    def fake_edit(
        backend, *, entity, view, query_type, add, remove, width, reorder, solution, publish
    ):
        called.update(entity=entity, view=view, add=add, remove=remove, width=width)
...
def test_set_order_bad_order_spec_errors(monkeypatch):
    ...
    res = CliRunner().invoke(
        ...
        [
            ...
            "My View",
            "--order",
            "name baddir",  # three parts → UsageError
        ],
    )
    assert res.exit_code != 0
...
def test_set_order_calls_core(monkeypatch):
    ...
    def fake_set(
        backend, *, entity, view, query_type, order, add_order, clear_order, solution, publish
    ):
        called.update(
            entity=entity, view=view, order=order, add_order=add_order, clear_order=clear_order
        )
        return {"savedqueryid": "1111" * 8, "name": view}
