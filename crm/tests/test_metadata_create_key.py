...
def test_create_entity_key_posts_and_returns_created(backend):
    ...
    with requests_mock.Mocker() as m:
        ...
        m.get(
            _key_url(backend, "account", "new_code"),
            status_code=404,
...
def test_create_entity_key_composite(backend):
    ...
    with requests_mock.Mocker() as m:
        m.get(
            _key_url(backend, "contact", "new_nameemail"),
            status_code=404,
            json={"error": {"code": "0x0", "message": "Not found"}},
        )
        post = m.post(
            _keys_url(backend, "contact"), status_code=204, headers={"OData-EntityId": "x"}
        )
        meta_mod.create_entity_key(
            backend,
            entity="contact",
            schema_name="new_NameEmail",
...
def test_create_entity_key_display_defaults_to_schema(backend):
    ...
    with requests_mock.Mocker() as m:
        ...
        post = m.post(
            _keys_url(backend, "account"), status_code=204, headers={"OData-EntityId": "x"}
        )
        meta_mod.create_entity_key(
            backend,
            entity="account",
...
def test_create_entity_key_dry_run_does_not_post(dry_backend):
    ...
    with requests_mock.Mocker() as m:
        m.get(
            _key_url(dry_backend, "account", "new_code"),
            status_code=404,
            json={"error": {"code": "0x0", "message": "Not found"}},
        )
        result = meta_mod.create_entity_key(
...
def test_create_entity_key_exists_error(backend):
    ...
    with requests_mock.Mocker() as m:
        m.get(_key_url(backend, "account", "new_code"), status_code=200, json={"MetadataId": "1"})
...
def test_cmd_create_key_json(runner, backend, monkeypatch):
    ...
    with requests_mock.Mocker() as m:
        ...
        m.post(_keys_url(backend, "account"), status_code=204, headers={"OData-EntityId": "x"})
