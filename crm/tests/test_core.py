...
class TestAlternateKeyPath:
    def test_single_string_key_is_quoted(self):
        path = entity_mod.build_alternate_key_path("contacts", {"emailaddress1": "joe@example.com"})
    ...
    def test_special_characters_are_url_escaped_and_quotes_doubled(self):
        path = entity_mod.build_alternate_key_path("accounts", {"accountnumber": "A/B C&D'E"})
...
class TestResolveAlternateKey:
        ...
        m.get(
            ...
            json={
                "value": [
                    ...
                    {
                        "LogicalName": "account_geo_ak",
                        "SchemaName": "Account_Geo_AK",
    ...
    def test_returns_matched_attributes_for_single_key(self, backend, isolated_home):
        with requests_mock.Mocker() as m:
            ...
            matched = entity_mod.resolve_alternate_key(backend, "accounts", ["accountnumber"])
...
class TestQuery:
    ...
    def test_odata_query_includes_annotations_prefer_header(self, backend):
        ...
        assert m.request_history[0].headers["Prefer"] == 'odata.include-annotations="*"'
...
class TestMetadata:
                    ...
                    "value": [
                        {
                            ...
                            "EntitySetName": "new_things",
                            "SchemaName": "new_Thing",
                            "IsCustomEntity": True,
                        },
                    ]
                },
    ...
    def test_list_attributes_selects_validity_fields(self, backend):
        ...
        for field in ("isvalidforcreate", "isvalidforupdate", "isvalidforread", "requiredlevel"):
...
class TestPublish:
    ...
    def test_publish_all_retries_on_lock_then_succeeds(self, backend, no_sleep):
        ...
        lock_err = {
            "error": {"code": "0x80071151", "message": "another [PublishAll] operation is running."}
        }
        with requests_mock.Mocker() as m:
            m.post(
    ...
    def test_publish_all_surfaces_original_error_after_exhausting_retries(
        ...
        with requests_mock.Mocker() as m:
            m.post(backend.url_for("PublishAllXml"), [{"status_code": 400, "json": lock_err}] * 10)
...
class TestExport:
    ...
    def test_export_honors_max_records_exactly(self, backend, tmp_path, max_records, expected):
...
class TestWorkflow:
    ...
    def test_set_workflow_state_resolves_activation_parent(self, backend):
        ...
        error_body = {
            "error": {"code": "0x80045003", "message": "Cannot update a workflow activation."}
        }
        with requests_mock.Mocker() as m:
            ...
            info = wf_mod.set_workflow_state(backend, wid, activate=False, caller_id=caller)
    ...
    def test_set_workflow_state_auto_resolve_opt_out(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.patch(backend.url_for(f"workflows({wid})"), status_code=400, json=error_body)
            with pytest.raises(D365Error) as exc_info:
                wf_mod.set_workflow_state(backend, wid, activate=True, auto_resolve_parent=False)
        assert exc_info.value.code == "0x80045003"
        assert [r.method for r in m.request_history] == ["PATCH"]
    ...
    def test_execute_workflow_posts_bound_action(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.post(
                backend.url_for(f"workflows({wid})/Microsoft.Dynamics.CRM.ExecuteWorkflow"),
    ...
    def test_activation_record_hint_fallback_when_no_parent(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.get(
                backend.url_for(f"workflows({wid})"),
                status_code=500,
                json={"error": {"code": "0x80000000", "message": "Server error"}},
            )
...
class TestExportSolutionAsync:
    ...
    def test_export_calls_async_then_poll_then_download(self, backend, tmp_path, monkeypatch):
        import time as _t
        ...
        with requests_mock.Mocker() as m:
            ...
            m.get(
                backend.url_for(f"asyncoperations({self.OP_ID})"),
                json={
                    "statecode": 3,
                    "statuscode": 30,
                    "message": "Done",
            ...
            m.post(
                backend.url_for("DownloadSolutionExportData"),
                json={
    ...
    def test_export_raises_on_async_failure(self, backend, tmp_path, monkeypatch):
        import time as _t
    ...
    def test_export_settings_flags_serialize_into_request_body(
        ...
        async_post = next(r for r in m.request_history if r.url.endswith("ExportSolutionAsync"))
...
class TestImportSolutionAsync:
    ...
    def test_import_calls_async_then_polls(self, backend, tmp_path, monkeypatch):
        import time as _t
    ...
    def test_import_skip_dependency_check_sets_flag(self, backend, tmp_path, monkeypatch):
    ...
    def test_import_default_omits_skip_dependency_check(self, backend, tmp_path, monkeypatch):
    ...
    def test_import_job_id_rejected_falls_back_to_sync_import(self, backend, tmp_path, monkeypatch):
    ...
    def test_sync_fallback_uses_long_read_timeout(self, backend, tmp_path, monkeypatch):
        ...
        def _run(timeout, expected):
            with requests_mock.Mocker() as m:
                m.post(
                    backend.url_for("ImportSolutionAsync"), status_code=400, json=self._REJECT_BODY
                )
                m.post(backend.url_for("ImportSolution"), status_code=204, text="")
    ...
    def test_sync_fallback_dependency_failure_raises_loudly(self, backend, tmp_path, monkeypatch):
        ...
        with requests_mock.Mocker() as m:
            m.post(backend.url_for("ImportSolutionAsync"), status_code=400, json=self._REJECT_BODY)
            m.post(
                backend.url_for("ImportSolution"),
                status_code=400,
                json={
                    "error": {
                        "code": "0x80048033",
                        "message": "The dependent component Entity (Id=foo) does not exist.",
                    }
                },
            )
            from crm.core import solution as sol_mod
    ...
    @pytest.mark.parametrize(
        "job_row_resp",
        [
            {"json": {"progress": 100.0, "startedon": None, "completedon": None, "data": None}},
            {"status_code": 404, "json": {"error": {"message": "importjob Does Not Exist"}}},
        ],
    ...
    def test_import_other_d365_error_reraised(self, backend, tmp_path, monkeypatch):
...
class TestLoadPayload:
    ...
    def test_malformed_data_json_raises_usage_error(self):
        ...
        with pytest.raises(click.UsageError, match=r"invalid JSON in --data: .*Expecting"):
    ...
    def test_malformed_data_file_raises_usage_error(self, tmp_path):
        ...
        with pytest.raises(click.UsageError, match=r"invalid JSON in --data-file: .*Expecting"):
