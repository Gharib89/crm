...
def _argv_map(tmp_path: Path, prefix: list[str]) -> dict[str, list[str]]:
    return {
        ...
        "entity:entity_clear_lookup": [
            *prefix, "--profile", "t", "entity", "clear-lookup", "accounts", _GUID, "primarycontactid"
        ],
        "metadata:metadata_delete_entity": [*prefix, "metadata", "delete-entity", "new_widget"],
        "metadata:metadata_delete_attribute": [*prefix, "metadata", "delete-attribute", "new_widget", "new_field"],
        "metadata:metadata_delete_key": [*prefix, "metadata", "delete-key", "account", "new_Code"],
        "metadata:metadata_delete_relationship": [*prefix, "metadata", "delete-relationship", "new_rel"],
        "metadata:metadata_delete_optionset": [*prefix, "metadata", "delete-optionset", "new_color"],
        "plugin:unregister_image_cmd": [*prefix, "plugin", "unregister-image", _GUID],
        "plugin:unregister_assembly_cmd": [*prefix, "plugin", "unregister-assembly", "Contoso.Plugins"],
        "plugin:unregister_step_cmd": [*prefix, "plugin", "unregister-step", "My Step"],
        "profile:profile_add": [
            *prefix, "profile", "add", "--url", _PROFILE_URL, "--domain", "CONTOSO",
        ...
        "ribbon:ribbon_hide_button": [
            *prefix, "ribbon", "hide-button", "account", "--target-id", "some.button",
            "--method", "hide-action", "--solution", "CRMWorx"
        ],
        "security:create_role": [*prefix, "security", "create-role", "R", "--solution", "mysol"],
        "security:set_role_privileges": [
            *prefix, "security", "set-role-privileges", "role-1",
            "--access", "read", "--entities", "account", "--depth", "global"
        ],
        "security:assign_role": [*prefix, "security", "assign-role", _GUID, "--to-user", _GUID2],
        "security:grant": [*prefix, "security", "grant", "accounts", _GUID, "--to", f"user:{_GUID2}", "--rights", "Read"],
        "security:revoke": [*prefix, "security", "revoke", "accounts", _GUID, "--from", f"user:{_GUID2}"],
        "solution:solution_remove_component": [
            *prefix, "solution", "remove-component", "--solution", "CRMWorx", "--type", "61", "--id", _GUID
        ],
        "solution:solution_uninstall": [*prefix, "solution", "uninstall", "--solution", "ContosoCore"],
        "solution:solution_stage_and_upgrade_cmd": [*prefix, "solution", "stage-and-upgrade", _zip_file(tmp_path, "upgrade.zip")],
        "solution:solution_apply_upgrade_cmd": [*prefix, "solution", "apply-upgrade", "ContosoCore"],
        "solution:solution_job_cancel": [*prefix, "solution", "job-cancel", _GUID],
        "solution:solution_import_cmd": [*prefix, "solution", "import", _zip_file(tmp_path, "import.zip")],
        "translation:translation_import_cmd": [*prefix, "translation", "import", _zip_file(tmp_path, "labels.zip")],
...
def _setup_case(case_id: str, monkeypatch, tmp_path: Path, called: dict) -> None:
    ...
    if case_id == "app:app_delete":
        monkeypatch.setattr("crm.commands.app.app_mod.delete_app", lambda *a, **k: _record(called, "app"))
    elif case_id == "apply:apply_cmd":
        monkeypatch.setattr(
            "crm.commands.apply.apply_mod.apply_spec",
            lambda *a, **k: _record(called, "apply", {
                "ok": True, "applied": [], "updated": [], "skipped": [],
        ...
        monkeypatch.setattr("crm.commands.solution.async_ops_mod.cancel_async_operation",
        ...
        monkeypatch.setattr("crm.commands.entity.entity_mod.disassociate",
                            lambda *a, **k: _record(called, "disassociate"))
    elif case_id == "entity:entity_clear_lookup":
        monkeypatch.setattr("crm.commands.entity.entity_mod.clear_lookup",
                            lambda *a, **k: _record(called, "clear"))
    elif case_id == "metadata:metadata_delete_entity":
        monkeypatch.setattr("crm.commands.metadata.meta_mod.delete_entity",
                            lambda *a, **k: _record(called, "meta-entity"))
    elif case_id == "metadata:metadata_delete_attribute":
        monkeypatch.setattr("crm.commands.metadata.ma_mod.delete_attribute",
                            lambda *a, **k: _record(called, "meta-attr"))
    elif case_id == "metadata:metadata_delete_key":
        monkeypatch.setattr("crm.commands.metadata.meta_mod.delete_entity_key",
                            lambda *a, **k: _record(called, "meta-key"))
    elif case_id == "metadata:metadata_delete_relationship":
        monkeypatch.setattr("crm.commands.metadata.rel_mod.delete_relationship",
                            lambda *a, **k: _record(called, "meta-rel"))
    elif case_id == "metadata:metadata_delete_optionset":
        monkeypatch.setattr("crm.commands.metadata.os_mod.delete_optionset",
                            lambda *a, **k: _record(called, "meta-os"))
    elif case_id == "plugin:unregister_image_cmd":
        monkeypatch.setattr("crm.commands.plugin.plugin_mod.unregister_image",
                            lambda *a, **k: _record(called, "plugin-image"))
    elif case_id == "plugin:unregister_assembly_cmd":
        monkeypatch.setattr("crm.commands.plugin.plugin_mod.unregister_assembly",
        ...
        monkeypatch.setattr("crm.commands.ribbon.ribbon_mod.retrieve_entity_ribbon",
                            lambda *a, **k: {"RibbonDiffXml": "<RibbonDiffXml />"})
        monkeypatch.setattr("crm.commands.ribbon.ribbon_mod.find_composed_element",
                            lambda *a, **k: {"Command": "Mscrm.Command"})
        monkeypatch.setattr("crm.commands.ribbon.ribbon_mod.apply_ribbon_change",
                            lambda *a, **k: _record(called, "ribbon-hide"))
    elif case_id == "security:create_role":
        monkeypatch.setattr("crm.commands.security.security_mod.create_role",
                            lambda *a, **k: _record(called, "create-role"))
    elif case_id == "security:set_role_privileges":
        monkeypatch.setattr("crm.commands.security.security_mod.set_role_privileges",
                            lambda *a, **k: _record(called, "set-role"))
    elif case_id == "security:assign_role":
        monkeypatch.setattr("crm.commands.security.security_mod.assign_role_to_user",
                            lambda *a, **k: _record(called, "assign"))
    elif case_id == "security:grant":
        monkeypatch.setattr("crm.commands.security.security_mod.grant_access",
                            lambda *a, **k: _record(called, "grant"))
    elif case_id == "security:revoke":
        monkeypatch.setattr("crm.commands.security.security_mod.revoke_access",
                            lambda *a, **k: _record(called, "revoke"))
    elif case_id == "solution:solution_remove_component":
        monkeypatch.setattr("crm.commands.solution.sol_mod.remove_solution_component",
                            lambda *a, **k: _record(called, "remove-component"))
    elif case_id == "solution:solution_uninstall":
        monkeypatch.setattr("crm.commands.solution.sol_mod.uninstall_solution",
                            lambda *a, **k: _record(called, "uninstall"))
    elif case_id == "solution:solution_stage_and_upgrade_cmd":
        monkeypatch.setattr("crm.commands.solution.sol_mod.import_solution",
                            lambda *a, **k: _record(called, "stage-upgrade"))
    elif case_id == "solution:solution_apply_upgrade_cmd":
        monkeypatch.setattr("crm.commands.solution.sol_mod.delete_and_promote",
        ...
        monkeypatch.setattr("crm.commands.translation.translation_mod.import_translation",
                            lambda *a, **k: _record(called, "translation"))
    elif case_id == "webresource:webresource_delete":
        monkeypatch.setattr("crm.commands.webresource.wr_mod.delete_webresource",
                            lambda *a, **k: _record(called, "webresource"))
    elif case_id == "workflow:workflow_deactivate":
        monkeypatch.setattr("crm.commands.workflow.workflow_mod.set_workflow_state",
        ...
        monkeypatch.setattr(
            ...
            lambda *a, **k: {
                "name": "Workflow", "workflow_id": _GUID,
                "resolved_from_activation_id": None,
            },
        )
        monkeypatch.setattr("crm.commands.workflow.workflow_mod.delete_workflow",
