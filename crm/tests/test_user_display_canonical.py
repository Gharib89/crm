...
def _assign_principal_type(flag: str, backend, monkeypatch) -> str:
    ...
    with rm_module.Mocker() as m:
        ...
        result = CliRunner().invoke(cli, [
            "--json", "fieldsec", "assign", _PROFILE_ID, flag, _USER_ID])
    assert result.exit_code == 0, result.output
...
def _create_display_name(flag: str, monkeypatch) -> str:
    ...
    with runner.isolated_filesystem():
        ...
        result = runner.invoke(cli, [
            "--json", "webresource", "create", "--name", "cwx_/foo.js",
            "--file", "foo.js", flag, "Pretty", "--no-publish", "--solution", "cwx_sol"])
    assert result.exit_code == 0, result.output
...
def test_canonical_flags_are_the_only_visible_spellings():
    ...
    for cmd in data["commands"]:
        ...
        assert "--display-name" not in _opts(cmd), \
            f"{cmd['path']} exposes hidden --display-name"

    # `--user-owned` is the canonical boolean on every chart verb that had `--user`.
    chart_user_verbs = [
        "chart list", "chart get", "chart delete", "chart create",
        "chart update", "chart set-fetch", "chart add-series",
