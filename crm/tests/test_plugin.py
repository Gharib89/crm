    ...
    def test_overrides_win_over_defaults(self, backend, tmp_path):
        ...
        with requests_mock.Mocker() as m:
            ...
            plugin.register_assembly(
                ...
                version="2.3.4.5",
                culture="en-US",
                public_key_token="0123456789abcdef",
            )
        body = _posts(m)[0].json()
    ...
    def test_missing_path_raises(self, backend):
        ...
        with requests_mock.Mocker() as m:
            with pytest.raises(D365Error):
                plugin.register_assembly(backend, path="/no/such/file/Contoso.Plugins.dll")
        assert m.request_history == []
...
class TestRegisterAssemblyUpdate:
    ...
    def test_update_resolves_by_name_override(self, backend, tmp_path):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.patch(backend.url_for(f"pluginassemblies({_PA_ID})"), status_code=204)
            plugin.register_assembly(backend, path=path, name="Other.Name", update=True)
        # resolves by the explicit name, not the filename stem
        assert m.request_history[0].qs["$filter"] == ["name eq 'other.name'"]

    def test_update_resolves_by_filename_stem_when_name_omitted(self, backend, tmp_path):
        from crm.core import plugin
    ...
    def test_update_solution_header_routed(self, backend, tmp_path):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.patch(backend.url_for(f"pluginassemblies({_PA_ID})"), status_code=204)
            out = plugin.register_assembly(backend, path=path, update=True, solution="Foo")
    ...
    def test_assembly_resolves_id_then_filters_by_assembly_value(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.get(
                ...
                json={"value": [{"plugintypeid": "aaa", "typename": "Contoso.Plugins.Foo"}]},
            )
...
class TestRegisterType:
    def test_resolves_assembly_then_posts_plugintype(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_type(
                backend, assembly="Contoso.Plugins", type_name="Contoso.Plugins.PreCreateAccount"
            )
        assert out["created"] is True
    ...
    def test_solution_header_routed(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_type(
                backend, assembly="Contoso.Plugins", type_name="Contoso.Plugins.Foo", solution="Foo"
            )
    ...
    def test_unknown_assembly_raises_clean_d365error(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.get(backend.url_for("pluginassemblies"), json={"value": []})
            with pytest.raises(D365Error, match="not found"):
                plugin.register_type(backend, assembly="Nope", type_name="X.Y")
    ...
    def test_unparseable_id_sets_lookup_error(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_type(backend, assembly="Contoso.Plugins", type_name="X.Y")
        assert out["created"] is True
...
def _mock_step_resolution(
    ...
    *,
    ...
    m.get(
        backend.url_for("sdkmessages"), json={"value": [{"sdkmessageid": _MSG_ID, "name": message}]}
    )
    m.get(
        backend.url_for("plugintypes"),
    ...
    if with_filter:
        m.get(
            backend.url_for("sdkmessagefilters"),
...
class TestRegisterStep:
    def test_with_entity_binds_all_three(self, backend):
        ...
        assert body["sdkmessagefilterid@odata.bind"] == f"/sdkmessagefilters({_FILTER_ID})"
        # default stage=postoperation(40), mode=sync(0), rank=1
        assert body["stage"] == 40
        assert body["mode"] == 0
        assert body["rank"] == 1
        # derived default name names type/message/entity
        assert body["name"] == "Contoso.Plugins.PreCreateAccount: Create of account"
        assert out["stage"] == 40
        assert out["mode"] == 0
    ...
    def test_without_entity_no_filter_bind_and_no_filter_get(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_step(
                backend, message="Create", plugin_type="Contoso.Plugins.PreCreateAccount"
            )
        body = _posts(m)[0].json()
        ...
        assert body["name"] == "Contoso.Plugins.PreCreateAccount: Create of any entity"
    ...
    def test_secure_configuration_deep_insert(self, backend):
        ...
        assert body["sdkmessageprocessingstepsecureconfigid"] == {"secureconfig": "s3cr3t"}
    ...
    def test_message_not_found_raises(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="[Mm]essage"):
                plugin.register_step(
                    backend, message="Nope", plugin_type="Contoso.Plugins.PreCreateAccount"
                )
        assert not _posts(m)
    ...
    def test_plugintype_not_found_raises(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="[Pp]lug-?in type"):
                plugin.register_step(
                    backend, message="Create", plugin_type="Contoso.Plugins.Missing"
                )
        assert not _posts(m)
    ...
    def test_service_endpoint_binds_eventhandler_not_plugintype(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.get(
                backend.url_for("serviceendpoints"), json={"value": [{"serviceendpointid": _SE_ID}]}
            )
            m.post(
                backend.url_for("sdkmessageprocessingsteps"),
                status_code=204,
            ...
            out = plugin.register_step(backend, message="Create", service_endpoint="Contoso Hook")
        body = _posts(m)[0].json()
        assert body["eventhandler_serviceendpoint@odata.bind"] == f"/serviceendpoints({_SE_ID})"
    ...
    def test_service_endpoint_not_found_raises(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="[Ss]ervice endpoint"):
                plugin.register_step(backend, message="Create", service_endpoint="Nope")
        assert not _posts(m)
    ...
    def test_service_endpoint_dry_run_reference(self, profile):
        ...
        with requests_mock.Mocker() as m:
            m.get(
                dry.url_for("sdkmessages"),
                json={"value": [{"sdkmessageid": _MSG_ID, "name": "Create"}]},
            )
            m.get(dry.url_for("serviceendpoints"), json={"value": []})
            out = plugin.register_step(dry, message="Create", service_endpoint="Ghost Hook")
        assert out["_dry_run"] is True
    ...
    def test_assembly_scopes_plugintype_lookup(self, backend):
        ...
        types = next(r for r in m.request_history if r.url.split("?")[0].endswith("plugintypes"))
...
class TestRegisterWebhook:
    def test_posts_serviceendpoint_with_webhook_fields(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.post(
                backend.url_for("serviceendpoints"),
                status_code=204,
    ...
    def test_auth_scheme_maps_to_authtype(self, backend):
        ...
        se_url = backend.url_for(f"serviceendpoints({_SE_ID})")
        for scheme, expected in [("webhookkey", 4), ("httpheader", 5), ("httpquerystring", 6)]:
            with requests_mock.Mocker() as m:
                m.post(
                    backend.url_for("serviceendpoints"),
                    status_code=204,
                ...
                plugin.register_webhook(
                    backend, name="H", url="https://e/h", auth=scheme, auth_value="v"
                )
    ...
    def test_unknown_auth_raises_no_http(self, backend):
        ...
        with requests_mock.Mocker() as m:
            with pytest.raises(D365Error, match="auth"):
                plugin.register_webhook(
                    backend, name="H", url="https://e/h", auth="bogus", auth_value="v"
                )
        assert m.request_history == []
    ...
    def test_dry_run_returns_preview_no_post(self, profile):
        ...
        with requests_mock.Mocker() as m:
            out = plugin.register_webhook(
                dry, name="H", url="https://e/h", auth="webhookkey", auth_value="v"
            )
        assert out["_dry_run"] is True
        assert not _posts(m)
    ...
    def test_unparseable_id_sets_lookup_error(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_webhook(
                backend, name="H", url="https://e/h", auth="webhookkey", auth_value="v"
            )
        assert out["created"] is True
...
class TestUnregisterStep:
    def test_resolves_by_name_then_deletes(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.delete(
                backend.url_for(f"sdkmessageprocessingsteps({_DELETE_STEP_ID})"), status_code=204
            )
    ...
    def test_ambiguous_name_raises_no_delete(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.get(
                ...
                json={
                    "value": [
                        {"sdkmessageprocessingstepid": _STEP_A},
...
class TestUnregisterAssembly:
    def test_deletes_dependent_steps_before_assembly(self, backend):
        ...
        with requests_mock.Mocker() as m:
            # assembly name -> id
            m.get(backend.url_for("pluginassemblies"), json={"value": [{"pluginassemblyid": aid}]})
            # plugintypes of the assembly
            m.get(
                backend.url_for("plugintypes"),
                json={"value": [{"plugintypeid": _PTID_A}, {"plugintypeid": _PTID_B}]},
            )
            ...
            m.delete(backend.url_for(f"sdkmessageprocessingsteps({_STEP_A})"), status_code=204)
            m.delete(backend.url_for(f"sdkmessageprocessingsteps({_STEP_B})"), status_code=204)
        ...
        assembly_idx = next(i for i, u in enumerate(del_urls) if f"pluginassemblies({aid})" in u)
        step_idxs = [i for i, u in enumerate(del_urls) if "sdkmessageprocessingsteps(" in u]
...
def _mock_image_resolution(m, backend, *, step_id=_STEP_ID, stage=20, message="Update"):
...
class TestRegisterImage:
    def test_pre_image_posts_required_fields_with_lowercase_bind(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.post(
                ...
                status_code=204,
                headers={"OData-EntityId": img_url},
            )
            out = plugin.register_image(backend, step=_STEP_ID, image_type="pre", alias="preimg")
        assert out["created"] is True
        assert out["sdkmessageprocessingstepimageid"] == _IMG_ID
        body = _posts(m)[0].json()
        # nav-prop is the lowercase logical name (issue #159 lesson)
        assert (
            body["sdkmessageprocessingstepid@odata.bind"]
    ...
    def test_step_resolved_by_exact_name(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_image(backend, step="My Step", image_type="pre", alias="preimg")
        assert out["created"] is True
        # resolved by exact name, selecting stage and message for derivation
        resolve = next(
            r
            for r in m.request_history
            if r.url.split("?")[0].endswith("sdkmessageprocessingsteps")
        )
        assert resolve.qs["$filter"] == ["name eq 'my step'"]
        # the bind targets the resolved id
        assert (
            _posts(m)[0].json()["sdkmessageprocessingstepid@odata.bind"]
            == f"/sdkmessageprocessingsteps({_STEP_ID})"
        )
    ...
    def test_post_image_on_postoperation_step(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_image(backend, step=_STEP_ID, image_type="post", alias="postimg")
        assert out["created"] is True
    ...
    def test_both_image_on_postoperation_step(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_image(backend, step=_STEP_ID, image_type="both", alias="bothimg")
        assert out["created"] is True
    ...
    def test_post_image_requires_postoperation_stage(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="[Pp]ost.*PostOperation"):
                plugin.register_image(backend, step=_STEP_ID, image_type="post", alias="postimg")
        assert not _posts(m)
    ...
    def test_pre_image_on_create_step_raises(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="[Pp]re-image.*Create"):
                plugin.register_image(backend, step=_STEP_ID, image_type="pre", alias="preimg")
        assert not _posts(m)
    ...
    def test_both_image_requires_postoperation_stage(self, backend):
        ...
        with requests_mock.Mocker() as m:
            _mock_image_resolution(m, backend, stage=20)
            with pytest.raises(D365Error, match="[Pp]ost.*PostOperation"):
                plugin.register_image(backend, step=_STEP_ID, image_type="both", alias="bothimg")
        assert not _posts(m)
    ...
    def test_explicit_name_wins_over_alias_default(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_image(
                backend, step=_STEP_ID, image_type="pre", alias="preimg", name="My Image"
            )
    ...
    def test_unknown_image_type_raises_no_http(self, backend):
        ...
        with requests_mock.Mocker() as m:
            with pytest.raises(D365Error, match="image type"):
                plugin.register_image(backend, step=_STEP_ID, image_type="bogus", alias="a")
        # validation happens before any HTTP call
        assert m.request_history == []
    ...
    def test_step_not_found_raises(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.get(backend.url_for("sdkmessageprocessingsteps"), json={"value": []})
            with pytest.raises(D365Error, match="not found"):
                plugin.register_image(backend, step="Nope", image_type="pre", alias="preimg")
        assert not _posts(m)
    ...
    def test_ambiguous_step_name_raises_no_post(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="Multiple plug-in steps"):
                plugin.register_image(backend, step="Dupe", image_type="pre", alias="preimg")
        assert not _posts(m)
    ...
    def test_unparseable_id_sets_lookup_error(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.post(
                backend.url_for("sdkmessageprocessingstepimages"),
                status_code=204,
                headers={"OData-EntityId": "https://x/sdkmessageprocessingstepimages(bogus)"},
            )
            out = plugin.register_image(backend, step=_STEP_ID, image_type="pre", alias="preimg")
        assert out["created"] is True
    ...
    def test_solution_header_routed(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.register_image(
                backend, step=_STEP_ID, image_type="pre", alias="preimg", solution="cwx_sol"
            )
...
class TestSetStepState:
    ...
    def test_dry_run_previews_no_patch(self, profile):
        ...
        with requests_mock.Mocker() as m:
            _mock_image_resolution(m, dry)
            m.patch(dry.url_for(f"sdkmessageprocessingsteps({_STEP_ID})"), status_code=204)
...
class TestUnregisterImage:
    def test_guid_deletes_directly(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.delete(backend.url_for(f"sdkmessageprocessingstepimages({_IMG_ID})"), status_code=204)
    ...
    def test_resolves_by_name_then_deletes(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.get(
                backend.url_for("sdkmessageprocessingstepimages"),
                json={"value": [{"sdkmessageprocessingstepimageid": _IMG_ID}]},
            )
            m.delete(backend.url_for(f"sdkmessageprocessingstepimages({_IMG_ID})"), status_code=204)
    ...
    def test_ambiguous_name_raises_no_delete(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="Multiple plug-in step images"):
    ...
    def test_dry_run_force_reads_id_no_delete(self, profile):
        ...
        with requests_mock.Mocker() as m:
            m.get(
                dry.url_for("sdkmessageprocessingstepimages"),
                json={"value": [{"sdkmessageprocessingstepimageid": _IMG_ID}]},
            )
...
class TestUnregisterCommands:
    ...
    def test_unregister_assembly_yes_skips_prompt(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda backend, assembly: (
                called.setdefault("a", assembly)
                or {
                    "deleted": True,
                    "pluginassemblyid": _PA_ID,
                    "steps_deleted": 0,
                    "deleted_step_ids": [],
                }
            ),
    ...
    def test_unregister_image_yes_skips_prompt(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda backend, image: (
                called.setdefault("i", image)
...
class TestPluginCommands:
    def test_register_assembly_command_wires_core(self, monkeypatch):
        ...
        def fake_register(backend, **kw):
            captured.update(kw)
            return {"created": True, "pluginassemblyid": _PA_ID, "name": "Contoso.Plugins"}
    ...
    def test_register_type_command_wires_core(self, monkeypatch):
        ...
        def fake_register_type(backend, **kw):
            captured.update(kw)
            return {"created": True, "plugintypeid": _PT_ID, "typename": kw["type_name"]}
    ...
    def test_register_type_command_handles_d365_error(self, monkeypatch):
        ...
        def boom(backend, **kw):
            raise D365Error("Plug-in assembly not found: Nope", code="PluginAssemblyNotFound")
    ...
    def test_register_step_command_defaults(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda backend, **kw: (
                captured.update(kw) or {"created": True, "sdkmessageprocessingstepid": _STEP_ID}
            ),
        )
        monkeypatch.setattr("crm.cli.CLIContext.backend", lambda self: object())
        result = CliRunner().invoke(
    ...
    def test_register_image_command_defaults(self, monkeypatch):
        ...
        monkeypatch.setattr(
            ...
            lambda backend, **kw: (
                captured.update(kw) or {"created": True, "sdkmessageprocessingstepimageid": _IMG_ID}
            ),
        )
        monkeypatch.setattr("crm.cli.CLIContext.backend", lambda self: object())
        result = CliRunner().invoke(
...
class TestUpdateStep:
    def test_patches_single_field_and_returns_fields_list(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.patch(
                backend.url_for(f"sdkmessageprocessingsteps({_UPDATE_STEP_ID})"), status_code=204
            )
    ...
    def test_patches_multiple_fields_body_correct(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = plugin.update_step(
                backend,
                step_id=_UPDATE_STEP_ID,
                stage="preoperation",
                mode="sync",
                filtering_attributes="name,email",
                configuration="<cfg/>",
            )
