...
class TestCreateView:
    ...
    def test_requires_columns(self, backend):
        ...
        with pytest.raises(D365Error, match="at least one column"):
            views.create_view(
                backend, entity="cwx_ticket", object_type_code=10042, name="X", columns=[]
            )
...
class TestEditViewColumns:
    def test_add_adds_cell_and_attribute_and_clears_layoutjson(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = views.edit_view_columns(
                backend, entity=_ENTITY, view="Active Tickets", add=[("cwx_priority", 150)]
            )
        body = _patch_body(m)
    ...
    def test_add_inserts_attribute_before_order(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            views.edit_view_columns(
                backend, entity=_ENTITY, view="Active Tickets", add=[("cwx_priority", 100)]
            )
    ...
    def test_add_validates_column_exists(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="does not exist"):
                views.edit_view_columns(
                    backend, entity=_ENTITY, view="Active Tickets", add=[("bogus", 100)]
                )
    ...
    def test_add_rejects_duplicate_column(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="already on the view"):
                views.edit_view_columns(
                    backend, entity=_ENTITY, view="Active Tickets", add=[("cwx_name", 100)]
                )
    ...
    def test_add_rejects_nonpositive_width(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="width must be positive"):
                views.edit_view_columns(
                    backend, entity=_ENTITY, view="Active Tickets", add=[("cwx_priority", 0)]
                )
    ...
    def test_remove_drops_cell_and_attribute(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            _mock_patch(m, backend)
            views.edit_view_columns(
                backend, entity=_ENTITY, view="Active Tickets", remove=["cwx_status"]
            )
        body = _patch_body(m)
    ...
    def test_remove_refuses_primary_key(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="primary-key"):
                views.edit_view_columns(
                    backend, entity=_ENTITY, view="Active Tickets", remove=[_PK]
                )
    ...
    def test_remove_missing_column_errors(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="not on the view"):
                views.edit_view_columns(
                    backend, entity=_ENTITY, view="Active Tickets", remove=["cwx_missing"]
                )
    ...
    def test_width_resizes_without_touching_fetch(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            _mock_patch(m, backend)
            views.edit_view_columns(
                backend, entity=_ENTITY, view="Active Tickets", width=[("cwx_name", 300)]
            )
        body = _patch_body(m)
    ...
    def test_reorder_permutes_columns(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            _mock_patch(m, backend)
            views.edit_view_columns(
                backend, entity=_ENTITY, view="Active Tickets", reorder=["cwx_status", "cwx_name"]
            )
        layout = _patch_body(m)["layoutxml"]
        assert layout.index('<cell name="cwx_status"') < layout.index('<cell name="cwx_name"')
    ...
    def test_reorder_must_be_a_permutation(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="exactly the current columns"):
                views.edit_view_columns(
                    backend, entity=_ENTITY, view="Active Tickets", reorder=["cwx_name"]
                )
    ...
    def test_no_op_errors(self, backend):
        ...
        with pytest.raises(D365Error, match="nothing to do"):
            views.edit_view_columns(backend, entity=_ENTITY, view="Active Tickets")
    ...
    def test_resolve_ambiguous_errors(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.get(backend.url_for("savedqueries"), json={"value": [_view_row(), _view_row()]})
    ...
    def test_resolve_by_guid_uses_direct_get(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.get(backend.url_for(f"savedqueries({_VIEW_ID})"), json=_view_row())
            m.patch(backend.url_for(f"savedqueries({_VIEW_ID})"), status_code=204)
            views.edit_view_columns(
                backend, entity=_ENTITY, view=_VIEW_ID, width=[("cwx_name", 300)]
            )
        # No collection probe — the GUID resolves directly.
        assert not any(
            r.method == "GET" and r.path.endswith("/savedqueries") for r in m.request_history
        )
    ...
    def test_dry_run_issues_no_patch(self, dry_backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            _mock_attr_ok(m)
            out = views.edit_view_columns(
                dry_backend, entity=_ENTITY, view="Active Tickets", add=[("cwx_priority", 150)]
            )
...
class TestAddViewFilter:
    def test_add_eq_condition_creates_filter(self, backend):
        ...
        assert out["conditions"] == [{"attribute": "cwx_status", "operator": "eq", "values": ["1"]}]
    ...
    def test_add_empty_conditions_errors(self, backend):
        ...
        with pytest.raises(D365Error, match="nothing to do"):
            views.add_view_filter(backend, entity=_ENTITY, view="Active Tickets", conditions=[])
...
class TestRemoveViewFilter:
    def test_remove_drops_matching_condition(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = views.remove_view_filter(
                backend, entity=_ENTITY, view="Active Tickets", conditions=[("statecode", "eq", [])]
            )
        fetch = _patch_body(m)["fetchxml"]
    ...
    def test_remove_empty_conditions_errors(self, backend):
        ...
        with pytest.raises(D365Error, match="nothing to do"):
            views.remove_view_filter(backend, entity=_ENTITY, view="Active Tickets", conditions=[])
    ...
    def test_remove_does_not_validate_attribute_existence(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            _mock_patch(m, backend)
            views.remove_view_filter(
                backend, entity=_ENTITY, view="Active Tickets", conditions=[("cwx_gone", "eq", [])]
            )
        fetch = _patch_body(m)["fetchxml"]
    ...
    def test_remove_publish_readback_verifies_failure(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.get(
                backend.url_for(f"savedqueries({_VIEW_ID})"), json={"fetchxml": _FETCH_WITH_FILTER}
            )
...
class TestSetViewOrder:
    def test_order_replaces_sort(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            views.set_view_order(
                backend, entity=_ENTITY, view="Active Tickets", order=[("createdon", True)]
            )
        fetch = _patch_body(m)["fetchxml"]
    ...
    def test_add_order_appends(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            _mock_patch(m, backend)
            views.set_view_order(
                backend, entity=_ENTITY, view="Active Tickets", add_order=[("createdon", False)]
            )
        fetch = _patch_body(m)["fetchxml"]
    ...
    def test_clear_order_removes_all(self, backend):
        ...
        with requests_mock.Mocker() as m:
            _mock_resolve(m, backend, _view_row())
            _mock_patch(m, backend)
            views.set_view_order(backend, entity=_ENTITY, view="Active Tickets", clear_order=True)
        fetch = _patch_body(m)["fetchxml"]
    ...
    def test_validates_order_attribute(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="does not exist"):
                views.set_view_order(
                    backend, entity=_ENTITY, view="Active Tickets", order=[("bogus", False)]
                )
    ...
    def test_no_op_errors(self, backend):
        ...
        with pytest.raises(D365Error, match="nothing to do"):
            views.set_view_order(backend, entity=_ENTITY, view="Active Tickets")
...
class TestParseHelpers:
    ...
    def test_layout_columns_round_trip(self):
        ...
        xml = build_layoutxml("cwx_ticket", 10042, [("cwx_name", 220), ("cwx_priority", 120)])
    ...
    def test_fetch_order_filter_none_when_absent(self):
        ...
        xml = build_fetchxml("cwx_ticket", [("cwx_name", 100)], order_by=None, filter_active=False)
...
class TestUpdateView:
    def test_dry_run_issues_no_patch(self, dry_backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = views.update_view(
                dry_backend, savedqueryid=_VIEW_ID, changes={"description": "x"}
            )
    ...
    def test_patches_changed_fields(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.patch(backend.url_for(f"savedqueries({_VIEW_ID})"), status_code=204)
            out = views.update_view(
                backend, savedqueryid=_VIEW_ID, changes={"isdefault": True, "description": "x"}
            )
        assert out["updated"] is True
