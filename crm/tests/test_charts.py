...
class TestRetargetChartxml:
    ...
    def test_protects_attribute_logical_names(self):
        ...
        xml = '<entity name="new_projectid"></entity><attribute name="new_project_code" />'
        out = retarget_chartxml(xml, src_entity="new_project", dst_entity="cwx_ticketclone")
...
class TestCloneChartToEntity:
    def test_posts_retargeted_chart(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.post(
                backend.url_for("savedqueryvisualizations"),
                status_code=204,
                headers={
                    "OData-EntityId": backend.url_for(
                        "savedqueryvisualizations(99998888-7777-6666-5555-444433332222)"
                    ),
                },
            )
...
class TestDeleteChart:
    ...
    def test_delete_dry_run_previews(self, dry_backend):
        ...
        assert out == {"_dry_run": True, "would_delete": True, "savedqueryvisualizationid": cid}
...
class TestCreateChart:
    ...
    def test_creates_user_chart_in_userset(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.post(
                _user_charts_url(backend),
                status_code=204,
                headers={
                    "OData-EntityId": backend.url_for(f"userqueryvisualizations({self._NEW_ID})")
                },
            )
            out = charts.create_chart(
                backend,
...
class TestAliasCoupling:
    ...
    def test_measurecollection_without_alias_raises(self):
        ...
        no_alias = _EDIT_DATA.replace('<measure alias="aggregate_column" />', "<measure />")
        with pytest.raises(D365Error, match="alias-coupling"):
    ...
    def test_single_cat_too_many_series_raises(self):
        from crm.core.charts import MAX_SERIES, _validate_alias_coupling
...
class TestUpdateChart:
    def test_partial_update_reads_other_column_and_patches_name(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            out = charts.update_chart(
                backend, _EDIT_ID, name="Renamed", data_description=_EDIT_DATA, publish=False
            )
        body = m.last_request.json()
    ...
    def test_rehoming_fetch_entity_rejected(self, backend):
        ...
        with requests_mock.Mocker() as m:
            m.get(_edit_url(backend), json=_EDIT_CHART)
            with pytest.raises(D365Error):
                charts.update_chart(backend, _EDIT_ID, data_description=rehomed, publish=False)
...
class TestAddRemoveSeriesOrchestration:
    ...
    def test_remove_series_patches_both_columns(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.patch(_edit_url(backend), status_code=204)
            out = charts.remove_chart_series(backend, _EDIT_ID, alias="series2", publish=False)
        body = m.last_request.json()
        assert "datadescription" in body and "presentationdescription" in body
...
class TestChartEditPublishGating:
    def test_user_chart_never_publishes(self, backend, monkeypatch):
        ...
        called = {}
        monkeypatch.setattr("crm.core.solution.publish_all", lambda b: called.setdefault("p", True))
        ...
        with requests_mock.Mocker() as m:
            ...
            with pytest.raises(D365Error, match="date column"):
                charts.set_chart_groupby(
                    backend, _EDIT_ID, column="new_priority", dategrouping="month", publish=False
                )
...
class TestLinkEntityValidation:
    ...
    def test_bad_link_entity_attribute_rejected(self, backend):
        ...
        with requests_mock.Mocker() as m:
            ...
            m.get(
                re.compile(r"EntityDefinitions\(LogicalName='new_project'\)"),
                json={"AttributeType": "String"},
            )
            m.get(
                re.compile(r"EntityDefinitions\(LogicalName='account'\)"),
                status_code=404,
                json={"error": {"message": "Not found"}},
            )
