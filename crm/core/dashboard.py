...
def delete_dashboard(backend: D365Backend, dashboard_id: str) -> dict[str, Any]:
    ...
    """
    dashboard_id = _normalize_dashboard_id(dashboard_id)
    row = as_dict(backend.get(f"{_FORM_SET}({dashboard_id})", params={"$select": "formid,type"}))
    _require_dashboard_type(dashboard_id, row.get("type"))
...
def _resolve_target_tab(root: ET.Element, tab: str | None) -> ET.Element:
    ...
    if tab is None:
        return tabs[0]
    target = next((t for t in tabs if t.get("name") == tab or _id_matches(t.get("id"), tab)), None)
    if target is None:
...
def _splice_tile_into_formxml(
    ...
    *,
    ...
    if _guid_counter(new_xml) != _guid_counter(formxml) + _guid_counter(
...
def _resolve_view(backend: D365Backend, view: str) -> tuple[str, str]:
    ...
    if vid is None:
        raise D365Error(f"--view must be a savedquery id (GUID): {view!r}")
    row = as_dict(
        backend.get(
            f"savedqueries({vid})", params={"$select": "savedqueryid,returnedtypecode,name"}
        )
    )
...
def _resolve_visualization(backend: D365Backend, chart: str, *, entity: str) -> str:
    ...
    if cid is None:
        raise D365Error(f"--chart must be a savedqueryvisualization id (GUID): {chart!r}")
    row = as_dict(
        backend.get(
...
def _commit_tile(
    ...
    *,
    ...
    """
    out: dict[str, Any] = {_ID_FIELD: dashboard_id, "name": dashboard.get("name"), "action": action}
    out.update({k: v for k, v in extra.items() if v is not None})
    return xml_edit.commit_xml_patch(
        backend,
        entity_set=_FORM_SET,
...
def add_chart_to_dashboard(
    ...
    *,
    ...
    return _commit_tile(
        ...
        publish=publish,
        solution=solution,
        extra={"view": view_id, "chart": vis_id, "entity": entity, "tab": tab, "section": section},
    )
...
def add_view_to_dashboard(
    ...
    *,
    ...
    if grid_mode is None:
        raise D365Error(f"--mode must be one of {', '.join(_VIEW_MODES)}; got {mode!r}.")
    info = get_dashboard(backend, dashboard_id)
    view_id, entity = _resolve_view(backend, view)
    params = {
        "TargetEntityType": entity,
    ...
    return _commit_tile(
        ...
        publish=publish,
        solution=solution,
        extra={"view": view_id, "entity": entity, "mode": mode, "tab": tab, "section": section},
    )
...
def _resolve_webresource(backend: D365Backend, ref: str) -> tuple[str, str | None]:
    ...
    if wid is not None:
        row = as_dict(
            backend.get(
                f"webresourceset({wid})", params={"$select": "webresourceid,name,webresourcetype"}
            )
        )
    else:
...
def remove_component_from_formxml(
    ...
    *,
    ...
    if len(selectors) != 1:
        raise D365Error(
            "remove-component needs exactly one of --cell-id / --index / --view / --chart / --url."
        )
    root = xml_edit.parse_xml(formxml, label="dashboard's FormXml")
    ...
    row_to_section = {r: s for s in root.iter("section") for r in s.findall("./rows/row")}
    ...
    if index is not None:
        ...
        matches = [
            c
            for c in components
            if _component_matches(c, cell_id=cell_id, view=view, chart=chart, url=url)
        ]
        if not matches:
    ...
    if _guid_counter(new_xml) + _guid_counter(removed_subtree) != _guid_counter(formxml):
        raise D365Error(
...
def remove_component_from_dashboard(
    ...
    *,
    ...
    new_xml, removed = remove_component_from_formxml(
        info["formxml"], cell_id=cell_id, index=index, view=view, chart=chart, url=url
    )
    return _commit_tile(
        backend,
        info[_ID_FIELD],
...
def create_dashboard(
    ...
    *formxml* is the dashboard layout XML, posted verbatim (authored in the
    ...
    if backend.dry_run:
        return {"_dry_run": True, "would_create": {"entity_set": _FORM_SET, "body": body}}
