...
@click.option("--state", default=None, help="ready | suspended | locked | completed | <int>")
@click.option(
    "--message", "message_name", default=None, help="Filter by messagename (e.g. ImportSolution)."
)
@click.option("--owner", "owner_id", default=None, help="Filter by systemuser GUID.")
@click.option("--top", type=int, default=50, help="Page size per call (default 50).")
@click.option(
    "--order-by",
    default="createdon desc",
    help="OData $orderby expression (default: 'createdon desc').",
)
...
@pass_ctx
def async_list(
    ctx: CLIContext, state, message_name, owner_id, top, order_by, filter, fetch_all, max_pages
):
