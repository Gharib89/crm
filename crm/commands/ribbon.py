...
def _diff_file_option(fn):
    ...
    return click.option(
        "--diff-file", "diff_file", default=None,
        ...
        help="Offline: apply this edit to a local RibbonDiffXml file (from "
             ...
             "`ribbon apply`. Mutually exclusive with --solution/--publish.")(fn)
...
def _check_offline_exclusive(diff_file: str | None) -> None:
    ...
    if cctx.get_parameter_source("publish") == ParameterSource.COMMANDLINE:
        raise click.UsageError(
            "--diff-file cannot be combined with --publish/--no-publish")
...
def ribbon_export(ctx: CLIContext, entity, application, solution, output):
    ...
    if solution:
        ...
        if ctx.dry_run:
            ...
            return
        with d365_errors(ctx):
            diff = ribbon_mod.load_solution_ribbon_diff(
                ctx.backend(), solution, entity)
...
def ribbon_add_button(ctx, entity, label, location, group_override, webresource,
                      ...
                      image32, publish, solution, diff_file):
    ...
    if diff_file is not None:
        ...
        ctx.emit(True, data={"button_id": ids.custom_action, "group": group,
                             "diff_file": diff_file}, warnings=None)
        return
...
def ribbon_remove(ctx, entity, button_id, yes, publish, solution, diff_file):
    ...
    if diff_file is not None:
        with d365_errors(ctx):
            ribbon_mod.edit_ribbon_diff_file(diff_file, mutate)
        ctx.emit(True, data={"removed": button_id, "diff_file": diff_file},
                 warnings=None)
        return
...
def ribbon_set_label(ctx, entity, button_id, label, tooltip_title,
                     tooltip_description, lcid, publish, solution, diff_file):
    ...
    if diff_file is not None:
        ...
        ctx.emit(True, data={"button_id": button_id, "label": label,
                             ...
                             "lcid": lcid, "diff_file": diff_file}, warnings=None)
        return
...
def ribbon_add_custom_rule(ctx, entity, command_id, webresource, function,
    ...
    if diff_file is not None:
        ...
        ctx.emit(True, data={"command_id": command_id, "rule_id": rule_id,
                             "diff_file": diff_file}, warnings=warnings or None)
        return
...
def ribbon_apply(ctx, entity, from_file, publish, solution):
    ...
    with d365_errors(ctx):
        ...
        result = ribbon_mod.apply_ribbon_change(
            ctx.backend(), solution=solution, entity=entity, mutate=mutate,
            publish=publish)
    ctx.emit(True, data={"entity": entity, "solution": solution,
                         "from": from_file, "result": result},
             warnings=None)
