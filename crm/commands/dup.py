...
@click.option(
    "--entity", default=None, help="Filter to rules whose base entity is this logical name."
)
@pass_ctx
def dup_list(ctx: CLIContext, entity: str | None) -> None:
    ...
    rows = [
        [
            r.get("name", ""),
            r.get("duplicateruleid") or "",
            r.get("baseentityname") or "",
            r.get("matchingentityname") or "",
...
@click.option(
    "--matching-entity", default=None, help="Matching entity logical name (defaults to ENTITY)."
)
@click.option("--description", default=None, help="Rule description.")
@_solution_option
@pass_ctx
def dup_create(ctx: CLIContext, entity, name, matching_entity, description, solution) -> None:
    ...
    with d365_errors(ctx):
        info = dup_mod.create_rule(
            ...
            name=name,
            entity=entity,
            matching_entity=matching_entity,
...
@click.argument("rule")
@click.option("--attr", "attribute", required=True, help="Base column logical name to compare.")
@click.option(
    "--operator",
    required=True,
    type=click.Choice(list(dup_mod.OPERATORS)),
...
@click.argument("rule")
@click.option("--wait", is_flag=True, help="Block until the async publish job completes.")
@click.option(
    "--timeout", type=int, default=None, help="Seconds to wait for the job (with --wait)."
)
@pass_ctx
...
@click.option(
    "--top", type=int, default=50, show_default=True, help="Maximum number of matches to return."
)
@pass_ctx
...
@pass_ctx
def dup_bulk_detect(
    ctx: CLIContext, entity, fetch_xml, fetchxml_file, job_name, wait, timeout
) -> None:
