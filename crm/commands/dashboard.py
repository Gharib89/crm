...
def _layout_options(fn):
    ...
    fn = click.option("--tab", default=None, help="Target tab (name or id; default: first tab).")(
        fn
    )
    fn = click.option(
        "--section",
        ...
        "a new section per tile (one component per section).",
    )(fn)
    fn = click.option(
        "--rowspan",
        ...
        default=1,
        show_default=True,
        help="Cell rowspan; the section is padded to match it.",
    )(fn)
    fn = click.option(
        "--colspan", type=click.IntRange(min=1), default=1, show_default=True, help="Cell colspan."
    )(fn)
    fn = click.option("--force", is_flag=True, help="Add beyond the default six-component cap.")(fn)
    return fn


@dashboard_group.command("add-chart")
@click.argument("dashboard_id")
@click.option("--view", required=True, help="savedquery id (GUID) whose data the grid shows.")
@click.option(
    "--chart",
    required=True,
    help="savedqueryvisualization id (GUID) to render; its primary entity must match the view's.",
)
@_layout_options
@_solution_option
@_publish_option
...
@click.argument("dashboard_id")
@click.option("--url", required=True, help="The IFRAME URL (must be non-empty).")
@click.option(
    "--security", is_flag=True, help="Restrict cross-frame scripting (the FormXml Security flag)."
)
...
@click.argument("dashboard_id")
@click.option(
    "--webresource", required=True, help="Web resource id (GUID) or unique name to embed."
)
@_layout_options
@_solution_option
@_publish_option
...
@click.argument("dashboard_id")
@click.option("--cell-id", "cell_id", default=None, help="Remove the component cell with this id.")
@click.option(
    "--index", type=int, default=None, help="Remove the component at this 0-based position."
)
@click.option(
    "--view", default=None, help="Remove the component whose ViewId is this savedquery id."
)
@click.option(
    "--chart", default=None, help="Remove the component whose VisualizationId is this id."
)
@click.option("--url", default=None, help="Remove the IFRAME/web-resource component with this Url.")
@_solution_option
@_publish_option
@pass_ctx
