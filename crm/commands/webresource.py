...
@click.option("--name", required=True, help="Web resource unique name, e.g. 'cwx_/scripts/foo.js'.")
@click.option(
    "--file",
    required=True,
...
@click.option(
    "--display", "display_name", default=None, help="Display name (defaults to the unique name)."
)
...
@click.option(
    "--display-name", "display_name_alias", default=None, hidden=True, help="Alias for --display."
)
@click.option(
    "--type",
    "wr_type",
    type=int,
    default=None,
    help="Override the D365 webresourcetype int; inferred from the file extension if omitted.",
)
@_solution_option
@_publish_option
@pass_ctx
def webresource_create(
    ctx: CLIContext, name, file, display_name, display_name_alias, wr_type, solution, publish
):
...
@click.option("--display", "display_name", default=None, help="New display name.")
@click.option(
    "--display-name", "display_name_alias", default=None, hidden=True, help="Alias for --display."
)
@_solution_option
@_publish_option
@pass_ctx
def webresource_update(
    ctx: CLIContext, name, file, display_name, display_name_alias, solution, publish
):
...
def webresource_delete(ctx: CLIContext, name, yes, check_dependencies):
    ...
    with d365_errors(ctx):
        info = wr_mod.delete_webresource(ctx.backend(), name, check_dependencies=check_dependencies)
...
@_publish_option
@pass_ctx
def webresource_push(ctx: CLIContext, directory, prefix, solution, publish):
    ...
    with d365_errors(ctx):
        res = wr_mod.push_webresources(
            ctx.backend(), directory, prefix=prefix, solution=solution, publish=publish
        )
    ok = not res["failed"]
    error = None
    if res["failed"]:
    ...
    keys = (
        ("would_create", "would_update", "skipped", "published")
        if res.get("_dry_run")
        else ("pushed", "updated", "skipped", "published")
    )
    ctx.emit(ok, data={k: res[k] for k in keys}, error=error, meta=meta, warnings=warnings)
...
def webresource_list(ctx: CLIContext, custom_only, top):
    """List web resources."""
    with d365_errors(ctx):
        items = wr_mod.list_webresources(ctx.backend(), custom_only=custom_only, top=top)
    if ctx.json_mode:
