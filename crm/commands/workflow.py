...
class _WorkflowCategoryType(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        ...
        self.fail(
            f"{value!r} is not a valid category. Use an integer or one of: {_CATEGORY_NAMES_LIST}.",
            param,
            ctx,
        )
...
class _WorkflowScopeType(click.ParamType):
    ...
    def convert(self, value, param, ctx):
        ...
        if resolved not in set(_SCOPE_NAMES.values()):
            self.fail(
                f"{value!r} is out of range — scope must be 1–4 (or one of: {_SCOPE_NAMES_LIST}).",
                param,
                ctx,
            )
        return resolved
...
@click.option(
    "--on-demand", "on_demand_only", is_flag=True, default=False, help="Only on-demand workflows."
)
@pass_ctx
...
def workflow_migration_assess(ctx: CLIContext, primary_entity):
    ...
    with d365_errors(ctx):
        backend = ctx.backend()
        items = workflow_mod.assess_workflow_migrations(backend, primary_entity=primary_entity)
...
def _activation_enrich(ctx: CLIContext, workflow_id):
    ...
    def _enrich(exc):
        hint = (
            workflow_mod.activation_record_hint(ctx.backend(), workflow_id, exc)
            if exc.code == workflow_mod.ACTIVATION_PATCH_ERROR_CODE
            else None
        )
        return hint, None
...
def workflow_activate(
    ctx: CLIContext, workflow_id, as_user, as_user_object_id, suppress_dup_detection, bypass_plugins
):
...
def workflow_delete(
    ...
    with d365_errors(ctx):
        info = workflow_mod.delete_workflow(ctx.backend(), workflow_id, resolved=target, **admin)
    ctx.emit(True, data=info, meta=_redirect_note_meta(info))
    _journal(ctx, workflow_id, info)
...
@click.option(
    "--scope",
    type=_WorkflowScopeType(),
    default=None,
    help=f"Execution scope. Integer (1–4) or a name: {_SCOPE_NAMES_LIST}.",
)
@click.option(
...
@click.option(
    "--on-create/--no-on-create",
    "trigger_on_create",
    default=None,
    help="Toggle the trigger-on-create flag.",
)
@click.option(
    "--on-delete/--no-on-delete",
    "trigger_on_delete",
    default=None,
