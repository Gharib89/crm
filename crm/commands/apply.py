...
def apply_cmd(
    ...
    if bool(spec_file) == bool(from_plan):
        raise click.UsageError("apply requires exactly one of -f/--file or --from-plan.")
    if from_plan:
        ...
        conflicting = [
            ...
            for name, on in (
                ("--prune", prune),
                ("--allow-data-loss", allow_data_loss),
                ("--stage-only", ctx.stage_only),
                ("-o/--plan-out", bool(plan_out)),
            )
            if on
        ]
        if conflicting:
    ...
    if not isinstance(spec, dict):
        ctx.emit(
            False, error="Spec must be a mapping (publisher / solution / entities / optionsets)."
        )
        return
    ...
    if prune:
        scope = (
            " (including data-bearing entities/attributes — destroys row data)"
            if allow_data_loss
            else "; data-bearing entities/attributes are skipped unless "
        ...
        _confirm_destructive(
            ...
            yes,
            message=(
                "--prune permanently DELETES components in the target solution "
                "that the spec no longer declares" + scope + ". This cannot be undone. Continue?"
            ),
        )
    ...
    with d365_errors(ctx):
        ...
        if plan_out:
            ...
            plan_doc = plan_mod.build_plan(
                ...
                solution=sol_block["unique_name"],
                base_dir=base_dir,
                prune=prune,
                allow_data_loss=allow_data_loss,
                stage_only=ctx.stage_only,
...
def _apply_data(res: dict) -> dict:
    ...
    return {
        k: res[k]
        for k in ("applied", "updated", "skipped", "replace_blocked", "pruned", "planned", "failed")
    }
...
def _apply_error_summary(res: dict) -> str | None:
    ...
    if res["failed"]:
        parts.append(
            "failed — "
            + "; ".join(
                f"{e['kind']} {e['name']}: {e.get('error', 'unknown error')}" for e in res["failed"]
            )
        )
...
def _apply_from_plan(
    ctx: CLIContext, plan_path: str, include_referenced_optionsets: bool, yes: bool
) -> None:
    ...
    with d365_errors(ctx):
        ...
        if plan_mod.plan_intent(plan_doc)["prune"] and not ctx.dry_run:
            _confirm_destructive(
                ...
                yes,
                message=(
                    "This plan's intent permanently DELETES components in the "
...
def _emit_plan_outcome(ctx: CLIContext, plan_path: str, outcome: dict, warnings: list[str]) -> None:
    ...
    if status == "stale":
        ...
        ctx.emit(
            False,
            data={"plan_valid": False, "divergences": divergences},
