...
named top-level keys (list rows, a single record, or — when every value is a list
of row-dicts and no requested field names a top-level key, e.g. `metadata
relationships` — inside each array, preserving the grouping), or the global
`--jq PROGRAM`
