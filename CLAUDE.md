...
- **SKILL ↔ CLI** — `crm/skills/` is the single tracked agent skill (source of truth): a thin `SKILL.md` router + `reference/*.md`. `crm skill install` copies the whole tree into a harness dir outside the repo (`~/.claude/skills/crm/`, etc.). Rules:
    ...
    - The tracked `.claude/skills/` tree holds two kinds of skill. **Project-native** ones (`ship`, `cloud-ship`, `merge-gate`, `live-e2e`, `audit-crm-skill`) have no *separate* source tree — `.claude/skills/` is itself their source of truth, hand-edited here (see `docs/agents/cloud-ship-routine.md`). **Vendored** ones (`tdd`, `code-review`, `writing-for-agents`, and the interactive toolkit) are *derived* copies of personal skills whose source of truth is `~/.claude/skills/`; run `python scripts/sync-skills.py` to refresh them and commit the result — **never hand-edit a vendored copy** (the next sync overwrites it). The tool copies each listed skill verbatim, strips `disable-model-invocation` for the cloud-composed ones (so the routine can invoke them), stamps `metadata.internal: true` on every copy (so `npx skills add Gharib89/crm` hides these dev skills from end users — revealed only with `INSTALL_INTERNAL_SKILLS=1`), and transitively pulls each skill's dependencies. Every `.claude/skills/*/SKILL.md` must carry that flag (project-native skills get it hand-added; a regression test in `crm/tests/test_skill_bundle.py` enforces it).
    - When editing `crm/skills/`, the docs-sync agent invokes the **`writing-for-agents`** skill (a vendored copy at `.claude/skills/writing-for-agents/`, refreshed by `scripts/sync-skills.py`) as the authority for skill structure and description rules.
...
Canonical labels: `needs-triage`, `needs-info`, `ready-for-agent`, `ready-for-human`, `wontfix`, plus `agent-working` (claimed by `/ship`). The `/triage` skill drives the front half — categorise an issue/external-PR through this label state machine (verify → grill → write an agent brief) up to `ready-for-agent`; `/ship` then drives the claim lifecycle from there. `/ship` phase 1 claims (`ready-for-agent` → `agent-working` + comment), phase 6 comments the PR link, merge closes. See `docs/agents/triage-labels.md`.

### Code review

Two reviewers run in lanes (decided in #833): **Copilot owns round 1, CodeRabbit owns iteration.** Copilot's round fires automatically; **every CodeRabbit round is explicitly triggered** (see below) — the lane split is unchanged, only CodeRabbit's trigger is manual.

**Copilot** auto-triggers on PR creation for ready (non-draft) PRs, via the repo ruleset *"Copilot auto review"* (target: default branch) — **round 1 fires automatically and is its only round in the ship flow; never re-request it.** Re-review does **not** trigger on push (`review_on_push: false`, deliberate). Its threads are dispositioned **once** — address, or decline with evidence. Review effort is the only model lever, and **no repo file controls it** — it lives in the repository's Copilot code-review settings, which an agent cannot read; the value recorded here is therefore *observed*, not confirmed: every recent review that prints a level reports **Lite** (#957, #959, #970). Treat that as the current behaviour and check the settings out of band before relying on a different tier. House rules + a known-non-issues list live in `.github/copilot-instructions.md` (only its first 4000 chars are read). The one place a Copilot re-request is still spent — an inbound agent PR the **merge-gate** itself significantly rewrote — is documented in the `merge-gate` skill, which now owns the REST re-request snippet and its 201-trap.

**CodeRabbit** does **not** auto-review this repo. Automatic review is star-gated and the repo is under CodeRabbit's 10-star threshold, so instead of a review it posts *"This repository does not receive automatic reviews because it has fewer than 10 stars"* with a trigger checkbox, and the run reports `Plan: Team` (not OSS) — `.coderabbit.yaml`'s `auto_review.enabled: true` is inert while that holds. It still **owns review iteration**, but every round is **requested, never push-triggered**: post `@coderabbitai review` as a PR comment **right after the PR is opened as ready**, and **again after each fix push** you want reviewed (still batch fixes into one push per round). The trigger only works **while the PR is open** — on a closed PR it declines ("Pull request is closed… does not re-review already reviewed commits"), so a round can never be recovered after merge. Config in `.coderabbit.yaml` (chill profile, agent PRs unfiltered). Engage its comments with the same rigor as Copilot's — address or decline with evidence, replying **on each review thread** ("fixed in `<sha>`" / decline + evidence). Once every thread carries a disposition, comment `@coderabbitai resolve` to close them all — never earlier (it bulk-resolves every CodeRabbit thread, dispositioned or not). A round costs only the trigger comment — but batch fixes one push per round anyway (rate limit 1–10 reviews/hr rolling, 150 files/review; `@coderabbitai rate limit` checks standing without spending one; PRs of roughly 300+ files get skipped by **both** bots). **Converged = the most recent *triggered* CodeRabbit round returned no new substantive threads (or CodeRabbit declined to review and that is recorded in the summary) + all Copilot round-1 threads dispositioned.** The "declined" branch is the defined exit when CodeRabbit refuses (closed PR, quota, outage) — **report it in the summary; never treat a refusal as quiet.** The shipping/gate summary splits review dispositions **per reviewer** (Copilot vs CodeRabbit) for lane accountability.
