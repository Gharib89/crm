...
**PRD/umbrella sub-issues are the exception to "a new command → `feat:`" — ship them as patches.** When an issue is a sub-task of an umbrella/PRD issue (its body links a parent, e.g. a `## Parent` section pointing at `#NNN`), title its PR `fix:`/`perf:` → **patch**, *even when it adds a whole command group*. A multi-issue PRD takes **one** minor bump, on the PR that **completes** it (the capstone — typically the one that closes the parent), titled `feat:` + the `minor` label; a minor bump zeroes the patch digit, so the capstone lands on the next `x.(y+1).0` regardless of how many sub-issue patches preceded it. Rationale: a PRD is one feature delivered across many issues — bumping minor per sub-issue is the v4.22 inflation this section exists to prevent. (`bump-guard` is the backstop: it blocks any `feat:`/breaking PR lacking the `minor`/`major` label, so an accidental minor can't merge unnoticed — but don't rely on it; pick the right type up front.)
