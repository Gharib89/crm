...
Two reviewers run in lanes (decided in #833): **Copilot owns round 1, CodeRabbit owns iteration.**

**Copilot** auto-triggers on PR creation for ready (non-draft) PRs, via the repo ruleset *"Copilot auto review"* (target: default branch) — **round 1 fires automatically and is its only round in the ship flow; never re-request it.** Re-review does **not** trigger on push (`review_on_push: false`, deliberate). Its threads are dispositioned **once** — address, or decline with evidence. Review effort is **Medium** (the only model lever); house rules + a known-non-issues list live in `.github/copilot-instructions.md` (only its first 4000 chars are read). The one place a Copilot re-request is still spent — an inbound agent PR the **merge-gate** itself significantly rewrote — is documented in the `merge-gate` skill, which now owns the REST re-request snippet and its 201-trap.

**CodeRabbit** reviews every ready PR (OSS plan; config in `.coderabbit.yaml` — chill profile, agent PRs unfiltered) and **owns review iteration**: unlike Copilot it **re-reviews automatically on every push** (incremental; auto-pauses after 5 reviewed commits — `@coderabbitai review` lifts the pause or forces a round). Engage its comments with the same rigor as Copilot's — address or decline with evidence, replying **on each review thread** ("fixed in `<sha>`" / decline + evidence). Once every thread carries a disposition, comment `@coderabbitai resolve` to close them all — never earlier (it bulk-resolves every CodeRabbit thread, dispositioned or not). CodeRabbit rounds are **free** and need no re-request — but batch fixes into one push per round anyway (OSS quota is 1–10 reviews/hr rolling, 150 files/review; `@coderabbitai rate limit` checks standing without spending one; PRs of roughly 300+ files get skipped by **both** bots). **Converged = CodeRabbit quiet on the latest push + all Copilot threads dispositioned.** The shipping/gate summary splits review dispositions **per reviewer** (Copilot vs CodeRabbit) for lane accountability.

### PR merge gate

Inbound agent-shipped PRs (cloud-ship routine, codex, teammates' agents) get a second, local review pass via the **`merge-gate`** skill before the maintainer merges: drift checklist + targeted live e2e + scoped fix-in-place + review-bot iteration (CodeRabbit re-reviews each push automatically; **one** Copilot re-request permitted only when the gate significantly rewrote the PR — the merge-gate exception to the round-1-only lane), ending in a `gate-passed`/`gate-failed` label. See `docs/agents/pr-merge-gate.md`.
