...
- `crm solution import` is now gated as a destructive operation: an overwrite
  ...
  prompts for confirmation and aborts cleanly (`{"ok": false, "error": "aborted
  by user"}`, exit 1) in a non-TTY context unless `--yes` is passed. The
  PreToolUse destructive-op gate also blocks any `crm solution import` without
