...
Deactivation changes server-side state (sets `statecode=0`). Pass `--yes` when calling non-interactively — omitting it in a non-TTY context (e.g. `--json` or piped) fails fast with the standard `ok:false` envelope, an error that names `--yes`, and exit 1. On an interactive terminal the verb prompts for confirmation instead.
