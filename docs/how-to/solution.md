...
`remove-component` is **destructive**: it prompts for confirmation unless `--yes`; in a non-TTY context it fails fast with the standard `ok:false` envelope, an error that names `--yes`, and exit 1. The agent-side PreToolUse hook also blocks it without `--yes` ([#71](https://github.com/Gharib89/crm/issues/71)).
