...
```
Saves the profile and runs a WhoAmI check. `--password` is accepted, but prefer `D365_PASSWORD` (via `.env` or a shell export) so the secret stays out of shell history; see the [CLI reference](../reference/cli.md).
