...
indexer, and while one of those holds a file open the move is refused. Every step
of the swap is therefore retried for a couple of seconds before it counts as a
failure, so a `self-update` on Windows may pause briefly. If a lock survives that
whole window,
