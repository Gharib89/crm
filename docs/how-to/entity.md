...
Both `create` and `update` accept `--return-record` (echo the row back) and `--no-return` (just the id) — only the default differs: `create` echoes unless you pass `--no-return`, `update` stays silent unless you pass `--return-record`. Passing both at once is a usage error.
