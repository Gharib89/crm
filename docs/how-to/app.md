...
```
`--unique-name` must carry the publisher prefix; `--if-exists skip` reports a skip with the existing `appmoduleid` instead of duplicating — this holds even when a just-created app isn't query-visible yet: the server's duplicate fault is treated as a skip, with a best-effort `appmoduleid` that may be `null` until the app is published. Creation **stages** by default (no publish); on-prem specifically, a freshly created appmodule isn't readable until it's published, so a staged `app create` commonly leaves an `app_lookup_error` advisory in `meta.warnings` on the read-back — pass `--publish` to publish before the read-back and avoid it. The required `webresourceid` icon comes from `--icon-webresource <name|guid>` (a name is resolved to its id, a GUID used directly, e.g. `--icon-webresource cwx_/icons/app.svg`); omit it to keep the platform default icon.
