...
Requested only when Copilot exits degraded (quota out, silent). CodeRabbit also reviews this repo on a `@coderabbitai review` comment, but it has no workflow for a round to be read off, which ship cannot express yet (Gharib89/skills#302), so it is not in this profile.
