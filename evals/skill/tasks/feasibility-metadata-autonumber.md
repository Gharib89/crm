...
# plug-in), so recognising the native, code-free path is the lift.
target: either
kind: feasibility
answer_key:
  cli_achievable: true
...
Task under assessment: *"The business wants every record in a table to get a human-readable
reference code that Dynamics generates automatically on save — formatted like `EVAL899-00001` (a
fixed prefix plus a zero-padded sequence) — without writing a plug-in or any custom code. Can this
be set up with the `crm` CLI, and if so, which command configures the auto-number column?"*
