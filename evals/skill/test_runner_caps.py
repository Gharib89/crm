...
#: Pass a real PATH so the subprocess resolves `sleep`/`printf` — env={} falls back to
#: os.defpath, which is platform-dependent and can miss them (a FileNotFoundError that
#: never exercises the timeout path).
_ENV = {"PATH": os.environ.get("PATH", "")}


def test_wall_clock_cap_trips_on_overrun(tmp_path):
    transcript, capped = _run_agent(
        "prompt", ["sleep", "5"], cwd=str(tmp_path), env=_ENV, wall_clock_s=1
    )
...
def test_normal_completion_is_not_capped(tmp_path):
    transcript, capped = _run_agent(
        "prompt", ["printf", "hello-world"], cwd=str(tmp_path), env=_ENV, wall_clock_s=30
    )
...
def test_no_wall_clock_never_caps(tmp_path):
    transcript, capped = _run_agent(
        "prompt", ["printf", "ok"], cwd=str(tmp_path), env=_ENV, wall_clock_s=None
    )
    assert capped is False
