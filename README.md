...
| `entity`     | Record CRUD (get/create/update/upsert/delete); `clone` (single-record clone, lookups rebound to the same parents, `--override`/`--unset`); `children` (per-1:N related-record counts via chunked `$batch`, not one query per relationship) |
