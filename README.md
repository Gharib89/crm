...
- `crm ribbon` to read and edit entity command-bar buttons (export / list / add-button / set-label / set-icon / remove / hide-button / set-rules / add-custom-rule) without manual solution-XML editing; button icons (Unified Interface SVG + classic 16×16/32×32 rasters) can be set on `add-button` or changed in place with `set-icon`; `ribbon export --application` exports the application-wide ribbon
