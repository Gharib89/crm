...
| `view`       | System views (savedquery): `list` / `create`; `edit-columns` to add, remove, resize, or reorder grid columns in place (keeps layoutxml + fetchxml coupled); `set-order` to replace, append, or clear the sort order — all without manual XML editing |
