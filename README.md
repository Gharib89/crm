...
| `chart`      | Author system and user charts headlessly: `create` from datadescription + presentationdescription XML files (or a `--web-resource` script visualization), `list <entity>`, `get <id>`, `delete <id>`; `--user` flag targets `userqueryvisualization` instead of `savedqueryvisualization`; `--publish` runs `PublishAllXml` |
