...
- `--jq PROGRAM` to run a jq program over any response client-side, so `--jq length solution list` returns a single count and `group_by`/`map` expressions produce census-style summaries instead of the full payload — same emit seam as `--fields` (mutually exclusive with it), implies `--json`, envelope-preserving
...
- These five flags — `--json`, `--fields`, `--jq`, `--profile`, `--dry-run` — are **dual-position**: valid before *or* after the subcommand, so `crm entity get accounts --json` is byte-identical to `crm --json entity get accounts` (position is pure syntax). Supplying one in both positions is a usage error. The other global flags stay root-only (before the command)
...
- **`crm/skills/`** — the agent skill loaded by skill-aware harnesses (Claude
  Code, Copilot CLI, …) after `crm skill install` (or, without the CLI,
  `npx skills add Gharib89/crm --skill crm -g -y`): a thin `SKILL.md` router plus
...
`meta` object. Parse `error` first and treat `meta` as optional.
...
| `apply`      | Declarative desired-state from a YAML/JSON spec (`apply -f spec.yaml`); declares publishers, solutions, entities (with columns, views, and **forms** — tabs / sections / field placement / JS libraries + handlers, converged onto the entity's main form), option sets, web resources, security roles, plug-in assemblies / types / steps / images, and **model-driven apps** (top-level `apps:` — components + sitemap as areas / groups / subareas); `--prune` opts in to solution-bounded deletion of org components the spec no longer declares (gated; preview with `--dry-run`) |
| `scaffold`   | Quick one-table shorthand: `scaffold table DISPLAY --column ...` creates an entity + N columns in one publish |
| `solution`   | List / info / components (`--save`/`--diff` for drift detection, `--resolve` to enrich each row with a friendly name + rootcomponentbehavior label) / audit (read-only AddRequiredComponents cascade / whole-entity drift report) / dependencies (uninstall-blocker preview) / missing-components (pre-import dependency check against the target org) / add-component / remove-component (both batchable — repeat `--id` or pass `--components-file`, run as one transactional `$batch`; an interactive `add-component` that would cascade in required components confirms first, `--yes` skips it) / set-version / export / import / import-result / extract / pack / validate solutions; `export-spec <unique_name> [-o FILE]` projects a whole solution (entities and their seedable main forms, security roles, web resources, and model-driven apps — identity + Entity-backed sitemap) into one `apply`-consumable desired-state spec (org-to-org drift recipe source); managed-upgrade lifecycle: clone-as-patch / stage-and-upgrade (holding import, `--promote`) / apply-upgrade (separate promote) / uninstall |
...
| `webresource` | Create/update/get/list/delete web resources (HTML/JS/CSS/images); stages by default, `--publish` to publish immediately; `get <name> --out <file>` backs up the current decoded bytes before an update (component-scoped rollback — no solution export/import cycle needed); `push <DIR> --prefix <p>` bulk-upserts a directory tree (name = `<prefix>_<relpath>`, type inferred from extension, skips byte-identical, `--publish` publishes once at the end); set as app icons |
...
| `form`       | Entity main forms (systemform): list, clone to another table, export formxml; add-field / remove-field / set-field / set-field-props to edit form layouts; add / remove / rename / move tab & section to edit the form's tab/section structure; add-library / add-handler / remove-handler / list-handlers to wire and inspect JS event handlers — all without manual FormXml editing; reads and writes warn that formxml labels are a single-language projection of the caller's `uilanguageid`, so the platform silently discards other-language labels on write; `labels <formid> --solution` dumps every element's labels across **all** provisioned languages by joining the form's structure to the solution's translation export (the label store), not that single-language projection |
...
| `dup`        | Duplicate-detection rules: create a rule, add match conditions, publish (async) / unpublish, `check` a candidate record against the published rules (`RetrieveDuplicates`), and `bulk-detect` — sweep a table for existing duplicates via a **server-side BulkDetectDuplicates async job**; list / get |
...
- `metadata export-spec <logical_name> [--with-views] [--with-relationships] [--with-forms] [-o FILE]` — project a live entity into a `crm apply -f`–consumable desired-state spec (round-trip). Pure GETs; `-o` writes the bare YAML directly. `--with-forms` projects the entity's seedable main form (custom-field placement, JS libraries + handlers) as a `forms:` block. Custom columns that cannot be represented (permission limits, unsupported formats) are reported in `meta.warnings` instead of silently dropped; a form that cannot round-trip a real apply (an additional main form) is reported in `meta.skipped`.
...
columns, option sets, relationships, views, forms, web resources, security roles,
plug-in assemblies with their types, steps, and images, and model-driven apps
with their sitemaps) from one declarative
