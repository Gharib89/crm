...
| `webresource` | Create/update/get/list/delete web resources (HTML/JS/CSS/images); stages by default, `--publish` to publish immediately; `push <DIR> --prefix <p>` bulk-upserts a directory tree (name = `<prefix>_<relpath>`, type inferred from extension, skips byte-identical, `--publish` publishes once at the end); set as app icons |
